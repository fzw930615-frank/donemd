...
DIST_DIR="dist"
# The .app wrapper filename = the system-facing name「马上做完」(PRODUCT_NAME in
# project.yml). macOS shows this — not CFBundleDisplayName — for the Dock tile /
# Finder / Spotlight, so the shipped bundle, the notarize target, the Sparkle
# update zip, and the dmg's drag-install app must all carry it. The Mach-O
# binary inside stays ASCII (Contents/MacOS/donemd, via EXECUTABLE_NAME).
APP_NAME="马上做完"
...
# `马上做完.app` itself as the single top-level entry, the exact shape of
...
echo "==> ditto: build .zip (Sparkle update artifact — 马上做完.app at root)"
# Stage the bundle back under its real name ${APP_NAME}.app (马上做完.app), NOT
